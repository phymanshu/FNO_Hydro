...
EOS_OUT=/eos/user/h/hsharma/vhlle_mcglauber_etascan/etas_${ETAS}/event_${JOBID}
MLDIR=/afs/cern.ch/work/h/hsharma/epos4/vhlle_prod

kinit -R 2>/dev/null; aklog 2>/dev/null
source /cvmfs/sft.cern.ch/lcg/views/LCG_106/x86_64-el9-gcc13-opt/setup.sh

WORKDIR=$(mktemp -d /tmp/mcglaub_etas_XXXXXX)
cd $WORKDIR
cp $VHLLE/hlle_visc .
cp -r $VHLLE/eos .
mkdir -p data

# generate MC Glauber IC with same b and seed as training
